...
var buildShareFile = function buildShareFile(_ref3) {
  ...
  var VTIMEZONEEntries = getVtimezoneFromMomentZone(timezone);
...
var getVtimezoneFromMomentZone = function getVtimezoneFromMomentZone() {
  var tzName = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : '';
  var MAX_OCCURENCES = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 2;
  if (tzName === '') return [];

  var zone = _momentTimezone.default.tz.zone(tzName);

  var header = "BEGIN:VTIMEZONE\nTZID:".concat(tzName);
  var footer = 'END:VTIMEZONE';
  var nMaxLoops = Math.min(MAX_OCCURENCES, zone.untils.length - 1);
  var zTZitems = zone.untils.slice(0, nMaxLoops).map(function (until, i) {
    var type = i % 2 === 0 ? 'STANDARD' : 'DAYLIGHT';

    var momDtStart = _momentTimezone.default.tz(until, tzName);

    var momNext = _momentTimezone.default.tz(zone.untils[i + 1], tzName);
  ...
  return [header].concat(_toConsumableArray(zTZitems), [footer]);
};
/**
